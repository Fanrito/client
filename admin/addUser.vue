<template>
    ...
    <!-- <n-space vertical hoverable class="bolck"> -->
      ...
      <p v-if="passwordsMatch===0" style="color: red;margin-bottom: 0;margin-top=0px;">密码不匹配</p>
        ...
        <n-form-item>
        <n-button @click="confirm" style="width: 200px;margin-left: 50px;margin-top: -10px;" type="primary" block secondary strong>
          确认
        </n-button>
        <br/>
        <n-button @click="reset_user" style="width: 200px;margin-left: 150px;margin-top: -10px;" type="primary" block secondary strong>
          重置
        </n-button>
      </n-form-item>
      </n-tab-pane>
...
function checkEmail() {
  ...
  if (emailRegex.test(email.value)) {
    ...
    return {
        name,showError,showOverlay,userPhoto,nick_name,phone,email,passwd,passwd2,campus,confirm,passwordsMatch,userNameMatch,userEmailMatch,
