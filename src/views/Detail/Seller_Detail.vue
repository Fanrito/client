...
<template>
    <div class="w">
      ...
      <n-layout has-sider>
        ...
        <n-layout-sider :native-scrollbar="false" width="300px" bordered >
          ...
          <n-button @click="editPhoto"  quaternary type="warning" style="align-items: center;margin-left: 100px;">编辑图片</n-button>
        ...
        <n-layout>
          <n-layout-header bordered>
            ...
            <div style="display: flex;margin-top: 20px;"><h3>商品名称：<n-input style="width: 350px;  margin-left: 10px;" :disabled="edit_disabled" v-model:value="res.data[0].goodsName"></n-input></h3></div>
            <div style="display:flex;flex-direction:row;align-items: center;">
            <h4 style="font-size:18;color:rgb(91, 91, 91);margin-bottom: 5px;">卖价
            <n-input-number size="small" style="width: 200px;margin-top: 5px;" v-model:value="res.data[0].curPrice" :disabled="edit_disabled"></n-input-number>
            </h4><span style="margin-left: 50px;"></span>
            <h4 style="font-size:18;color:rgb(91, 91, 91);margin-bottom: 5px;">原价
            <n-input-number size="small" style="width: 200px;margin-top: 5px;" v-model:value="res.data[0].oriPrice" :disabled="edit_disabled"></n-input-number></h4>

            </div>
            <div align="right" style="margin-top: 20px;"><font color="grey">库存 {{ res.data[0].inventory }}</font></div>
          </n-layout-header>
          <n-layout-content content-style="padding: 24px;">
            <!-- <Bottom></Bottom><br><br> -->
            <h4 style="margin-top: 0px;">商品描述</h4>
            <n-input style="height: 130px;" v-model:value="res.data[0].goodsProfile" :disabled="edit_disabled"/>
          </n-layout-content>
          <n-layout-footer bordered>
            <NDialogProvider>
              <SellerInfoChoose :inventory="res.data[0].inventory"  @update_inventory="receive" @edit="Edit" @confirm="confirm"></SellerInfoChoose>
            </NDialogProvider>
          </n-layout-footer>
        </n-layout>
      </n-layout>
  ...
  import SellerInfoChoose from './seller_InfoChoose.vue';
  // import { NDialogProvider } from 'naive-ui';
  import {ref,onMounted,inject} from 'vue';
  const axios = inject('axios');
  const edit_disabled=ref(true);
  function Edit()
  {
  ...
  function receive(new_inventory)
  {
    res.value.data[0].inventory=new_inventory
   console.log(res.value.data[0].inventory)
  }
  function editPhoto()
  {

   router.push('/editPhoto') 
  }
  let res=ref({
    "code": 1,
    "msg": "success",
