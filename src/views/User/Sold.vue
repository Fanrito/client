...
const loadPublishedGoods = async () => {
  ...
  if (res.data.data.code == 1) {
    ...
    // }
    let goodsInfo = {
      imgSrc: res.data.data.goodsImage,
      goodsId: res.data.data.goodsId,
      goodsName: res.data.data.goodsName,
      oriPrice: res.data.data.oriPrice,
      curPrice: res.data.data.res.data.data,
      goodsCategoryName: res.data.datagoodsCategoryName,
      releaseTime: res.data.data.releaseTime,
      inventory: res.data.data.inventory,
      goodsProfile: res.data.data.goodsProfile,
      linkHref: `/seller_detail/goods?goodsId=${goodsInfo.goodsId}`
    }
    publishedGoodsList.push(goodsInfo)
  }
}
</script>
